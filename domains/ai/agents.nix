...
let
  ...
  # Capability names used by the Pi configuration.
  allCapabilities = [
    "read"
    "grep"
    "find"
  ...
  # ---------------------------------------------------------------------------
  # Registry. Prompt bodies live in ./agents/<name>.md; everything policy-
  # related lives here.
  #
  ...
  #   permissions  argument-level bash and path rules shared by both harnesses
  # ---------------------------------------------------------------------------
  ...
  # (/name) while subagents become subagent definitions (~/.pi/agent/agents).
