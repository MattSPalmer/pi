...
writeFileSync(
  ...
  JSON.stringify({
    bash: {
      ...
      "gh api * --method *": {
        action: "deny",
        context: "mutating gh api is not permitted",
      },
      "gh api * --method=*": {
        action: "deny",
        context: "mutating gh api is not permitted",
...
const { normalizedCommandRule } = await import("./bash-rules.ts");

const rules = loadPolicyRules(cwd, home);

test("classifies allow, deny, ask, and unrecognized commands", () => {
  expect(
    classifyCommand({ command: "sort", cwd, bashRules: rules.bash }).kind,
  ).toBe("allow");
  const denied = classifyCommand({
    command: "git status",
    cwd,
    bashRules: rules.bash,
  });
  expect(denied.kind).toBe("deny");
  expect(denied.reason).toContain("use jj instead");
  expect(
    classifyCommand({ command: "rm file", cwd, bashRules: rules.bash }).kind,
  ).toBe("ask");
  expect(
    classifyCommand({ command: "unknown-tool", cwd, bashRules: rules.bash })
      .kind,
  ).toBe("unrecognized");
});
...
test("namespaced command policies are executable-agnostic", () => {
  expect(
    classifyCommand({ command: "jj status", cwd, bashRules: rules.bash }).kind,
  ).toBe("allow");
  expect(
    classifyCommand({ command: "jj describe", cwd, bashRules: rules.bash })
      .kind,
  ).toBe("ask");
  expect(
    classifyCommand({ command: "hg status", cwd, bashRules: rules.bash }).kind,
  ).toBe("allow");
  expect(
    classifyCommand({ command: "hg commit", cwd, bashRules: rules.bash }).kind,
  ).toBe("unrecognized");
});

test("normalizes jj global options before every policy lookup", () => {
  const jjRead = { glob: "jj diff*", action: "allow" as const };
  const jjAsk = { glob: "jj status*", action: "ask" as const };

  expect(
    normalizedCommandRule(
      [jjRead],
      ["jj", "-R", "workspace", "diff", "--", "file"],
    ),
  ).toBe(jjRead);
  expect(
    normalizedCommandRule(
      [jjAsk],
      ["jj", "--repository=workspace", "--no-pager", "status"],
    ),
  ).toBe(jjAsk);
});
test("classifies opaque syntax and strips redundant working-directory prefixes", () => {
  ...
  expect(
    classifyCommand({ command: "echo $(pwd)", cwd, bashRules: rules.bash })
      .kind,
  ).toBe("opaque");
  expect(
    classifyCommand({
      command: "cat ~/.ssh/id_rsa $(pwd)",
      cwd,
      bashRules: rules.bash,
    }).kind,
  ).toBe("deny");
...
test("classifies opaque interpreter variants", () => {
  for (const command of [
    ...
    expect(classifyCommand({ command, cwd, bashRules: rules.bash }).kind).toBe(
      "interpreter",
    );
  }
});
...
test("gh api mutation variants are denied while reads are not", () => {
  ...
  for (const command of denied) {
    expect(evaluateAllowlist({ command, cwd, rules }).verdict, command).toBe(
      "deny",
    );
  }
  expect(
    evaluateAllowlist({ command: "gh api repos/acme/project", cwd, rules })
      .verdict,
  ).not.toBe("deny");
});
...
test("sensitive path boundaries do not overmatch", () => {
  expect(
    checkPathAgainstRules(join(home, ".ssh", "id_rsa"), cwd, rules.paths)
      .status,
  ).toBe("deny");
  expect(
    checkPathAgainstRules(join(home, ".sshx", "id_rsa"), cwd, rules.paths)
      .status,
  ).toBe("unknown");
});
...
test("falls back to user defaults when an override config dir has no defaults", () => {
  ...
  try {
    ...
    expect(
      evaluateAllowlist({ command: "sort", cwd, rules: active }).verdict,
    ).toBe("allow");
  } finally {
...
test("delegated-agent policy is applied as the highest-precedence layer", () => {
  ...
  writeFileSync(
    join(configDir, "permissions", "restricted.json"),
    JSON.stringify({
  ...
  try {
    const active = loadPolicyRules(cwd, home);
    expect(
      evaluateAllowlist({ command: "sort input", cwd, rules: active }).verdict,
    ).toBe("deny");
  } finally {
    if (previousConfig === undefined) delete process.env.PI_CONFIG_DIR;
    else process.env.PI_CONFIG_DIR = previousConfig;
...
test("checks policy paths and persisted session grants without UI", () => {
  expect(
    checkPathAgainstRules(join(cwd, "input.txt"), cwd, rules.paths, [cwd])
      .status,
  ).toBe("allow");
  expect(
    checkPathAgainstRules(join(home, ".ssh", "id_rsa"), cwd, rules.paths)
  ...
  expect(checkPathAgainstRules(outside, cwd, rules.paths).status).toBe(
    "unknown",
  );
  expect(
    checkPathAgainstRules(outside, cwd, rules.paths, [join(root, "outside")])
      .status,
  ).toBe("allow");
});

test("allowlist evaluation requires positive command and path coverage", () => {
  expect(evaluateAllowlist({ command: "sort", cwd, rules })).toMatchObject({
    verdict: "allow",
  });
  expect(
    evaluateAllowlist({ command: "git status", cwd, rules }),
  ).toMatchObject({ verdict: "deny" });
  expect(evaluateAllowlist({ command: "rm file", cwd, rules })).toMatchObject({
    verdict: "needs-confirmation",
  });
  expect(
    evaluateAllowlist({
      command: `cat ${join(root, "outside", "file.txt")}`,
      cwd,
      rules,
    }),
  ).toMatchObject({
...
test("session command grants allow bounded command prefixes", () => {
  const grants = sessionGrantsFromEntries([
    ...
    },
    {
      type: "custom",
      customType: "permission-gate-path-grant",
      data: { path: join(root, "outside") },
    },
    {
      type: "custom",
      customType: "permission-gate-path-grant",
      data: { command: "x".repeat(4097) },
    },
